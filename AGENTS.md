...
- `Public/World`, `Private/World`: volúmenes de mundo e interactuables (`TN_FinishLineVolume`, `TN_DeathZoneVolume`, `TN_InteractableBase`, `TN_DirectInteractableBase`, `TN_PickupInteractableBase`, `TN_StaminaBoostPickup`, `TN_ThrowableItemActor`, `TN_CosmeticsStationInteractable`, `TN_ButtonInteractable`, `TN_ItemSpawnZone`, `TN_PufferFishActor`, `TN_RescuePickup`).
  - **`TN_ButtonInteractable`** (hereda `TN_DirectInteractableBase`): botón toggle que mueve un actor o componente target aplicando un offset de transform. Al pulsar, suma `ActivatedOffset` al transform original del target; al volver a pulsar, lo devuelve a la posición original. `bIsActivated` (bool) replicado; tanto servidor como clientes interpolan el target localmente en `Tick` hacia el destino (no se usa `SetReplicateMovement(true)`). Multicast unreliable para feedback cosmético (sonido/VFX en BP). **Chunk-compatible**: `MoveTargetTag` (FName) busca el target **dentro del chunk BP padre** en 4 pasos:
  ...
  **ActivatedOffset** (FTransform, configurable en BP): offset **aditivo** (suma simple de Location, composición de Rotation) que se aplica al transform original del target en `DeferredInit`. `ActivatedTransform.Location = Original.Location + Offset.Location`. **NO usar multiplicación de FTransform** (escalaba el offset por la Scale3D del target → valores absurdos con chunks). Timers diferidos usan `FTimerDelegate::CreateUObject` (no lambdas) para que `EndPlay→ClearAllTimersForObject` los cancele si el actor es destruido (ej. chunk temporal del ChunkManager).
...
- **`RevivePlayer` actualizado**: maneja tanto DBNO como jugadores muertos. Busca pawn en `DeadPlayerPawns` si `GetPawn()` es null. Restaura visibilidad del pawn (`SetActorHiddenInGame(false) + SetActorEnableCollision(true)`), `bIsAlive`, `bHasFinishedRun=false`, `bIsEliminated=false`. Llama `SetDeadVisual(false)` + `RecoverFromKnockdown()`. **Fix listen-server**: fuerza `UnPossess()` antes de `Possess(Pawn)` si el PC aún posee al pawn (en el listen-server, `BeginSpectatingState` puede NO llamar `UnPossess`, haciendo que `Possess(Pawn)` sea un no-op y `ChangeState(Playing)` nunca se ejecute). Fuerza `ChangeState(NAME_Playing)` como safety net. Llama `SetViewTarget(Pawn)` para limpiar el viewtarget del espectador. Re-posee el pawn y restaura input/movement. Otorga inmunidad temporal. Destruye el pickup de rescate. Limpia `DeadPlayerPawns`.
...
## Death zones
- `TN_DeathZoneVolume` hereda `AActor` con `UBoxComponent` (refactorizado desde `ATriggerVolume` para ser chunk-compatible). No mata instantáneamente: usa un countdown por jugador (`SecondsInsideToDie`, default 3s) con ticks cada `CountdownTickInterval` (default 0.1s).
