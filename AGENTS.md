...
- `Public/World`, `Private/World`: volúmenes de mundo e interactuables (`TN_FinishLineVolume`, `TN_DeathZoneVolume`, `TN_InteractableBase`, `TN_DirectInteractableBase`, `TN_PickupInteractableBase`, `TN_StaminaBoostPickup`, `TN_ThrowableItemActor`, `TN_CosmeticsStationInteractable`, `TN_ButtonInteractable`, `TN_ItemSpawnZone`, `TN_PufferFishActor`, `TN_RescuePickup`).
  ...
  **ActivatedOffset** (FTransform, configurable en BP): offset relativo que se compone con el transform original del target en `DeferredInit`. `ActivatedTransform = ActivatedOffset * OriginalTransform`. El offset es relativo al espacio local del target, así que funciona correctamente sin importar dónde se coloque el chunk en el mundo. Timers diferidos usan `FTimerDelegate::CreateUObject` (no lambdas) para que `EndPlay→ClearAllTimersForObject` los cancele si el actor es destruido (ej. chunk temporal del ChunkManager).
